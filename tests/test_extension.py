...
class ExtensionTests(unittest.TestCase):
    ...
    def test_capture_and_recording_contract_is_present(self):
        ...
        self.assertIn("this.connect('button-press-event'", source)
        self.assertNotIn("this._icon.connect('button-press-event'", source)
        self.assertNotIn("reactive: true", source)
        self.assertIn("button === Clutter.BUTTON_MIDDLE", source)
        self.assertIn("const DEFAULT_RECORDING_DURATION = 5", source)
        self.assertIn("this._selectedRecordingDuration = DEFAULT_RECORDING_DURATION", source)
        self.assertIn("this._startRecording(this._selectedRecordingDuration)", source)
        self.assertIn("button === Clutter.BUTTON_SECONDARY", source)
        self.assertIn("return Clutter.EVENT_STOP;", source)
        self.assertIn("'GIF Duration'", source)
        self.assertIn("_setRecordingDuration(duration)", source)
        self.assertIn("PopupMenu.Ornament.DOT", source)
        self.assertIn("label: 'Set Duration'", source)
        self.assertIn("'How to Use Grab'", source)
        self.assertIn("'How to use Grab'", source)
        self.assertIn("`Left-click opens this menu to set the GIF duration.", source)
        self.assertNotIn("() => this._startRecording(duration)", source)
